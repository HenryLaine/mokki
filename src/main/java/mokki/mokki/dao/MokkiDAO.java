...
public class MokkiDAO {
    ...
    public List<RaportitWrapper> haeRaportti(LocalDate alku, LocalDate loppu) {
        ...
        SELECT 
            ...
            ROUND(100 * SUM(DATEDIFF(LEAST(v.paattymis_pvm, ?) , GREATEST(v.aloitus_pvm, ?)) + 1) / (DATEDIFF(?, ?) + 1), 0) AS kayttoaste_prosentti,
            m.hinta,
            ROUND(m.hinta, 2) AS paivatulot,
            ROUND(m.hinta * 7, 2) AS viikkotulot,
            ROUND(m.hinta * 30, 2) AS kuukausitulot,
            ROUND(SUM((DATEDIFF(LEAST(v.paattymis_pvm, ?) , GREATEST(v.aloitus_pvm, ?)) + 1) * m.hinta), 2) AS kokonaistulot
        FROM Mokki m
        LEFT JOIN Varaus v ON m.mokkiID = v.mokkiID 
            AND v.aloitus_pvm <= ? AND v.paattymis_pvm >= ?
    ...
    """;
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    String kohde = rs.getString("kohde");
                    int kayttoaste = rs.getInt("kayttoaste_prosentti");
                    int varaustenMaara = rs.getInt("varausten_maara");
                    double paivatulot = rs.getDouble("paivatulot");
                    double viikkotulot = rs.getDouble("viikkotulot");
                    double kuukausitulot = rs.getDouble("kuukausitulot");
                    double kokonaistulot = rs.getDouble("kokonaistulot");

                    RaportitWrapper wrapper = new RaportitWrapper(
                            kohde, kayttoaste, varaustenMaara,
                            paivatulot, viikkotulot, kuukausitulot, kokonaistulot
                    );

                    raportti.add(wrapper);
                }
            }
